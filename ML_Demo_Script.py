...
def print_instances_from_data(data, features, instance, answer):
    
    my_sample = data[features].loc[instance].values
    my_answer = data['left_company'].loc[instance]
    print('\n')
    for i in range(len(features)):
        print(f'{features[i]} is {my_sample[i]}')
    if answer:
        print('\n')
        if my_answer == 0:
            print('The employee did not leave the company.')
        else:
            print('The employee left the company.')
            
def print_instances_from_test(X_test, y_test, best_model, features, instance, answer):
    import numpy as np
    ...
    pred = int(best_model.predict(np.array(X_test)[instance,:].reshape(1,-1)))
    actual = np.array(y_test)[instance]
    print('\n')

    if answer:
        if pred == 1:
            print('The model predicts that the employee left.')
        else:
            print('The model predicts that the employee stayed.')

        if actual == 1:
            print('The employee actually left.')
        else:
            print('The employee actually stayed.')
